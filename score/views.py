...
def dao(request):
    return render(request,'dao.html',{'name':'Compound Dao'})
...
def set_chart(request):
    if request.method == 'POST':
        df = get_data()
        data = {
            'vote_data':get_graph_values(get_voting_data(df)[0],get_voting_data(df)[1],get_voting_data(df)[2],get_voting_data(df)[3],get_voting_data(df)[4]),
            'voter_data':get_graph_values(get_voter_graph(df)[0],get_voter_graph(df)[1],get_voter_graph(df)[2],get_voter_graph(df)[3],get_voter_graph(df)[4]),
            'praposals_data':get_graph_values(get_praposals_data(df)[0],get_praposals_data(df)[1],get_praposals_data(df)[2],get_praposals_data(df)[3],get_praposals_data(df)[4]),
...
def get_voter_graph(df):
    ...
    df_final_VoteCast = df_VoteCast[['block_signed_at','block_height','tx_hash','voter','proposalId','support','votes']]
    ...
    return (address[0:10],votes_count[0:10],voted_for_count[0:10],'Total Vote','Numbers of Praposals Voted For')
...
def get_praposals_data(df):
    ...
    df_final_ProposalCreated = df_ProposalCreated[['block_signed_at','block_height','tx_hash','id','proposer','description']]
    ...
    df_final_ProposalExecuted = df_ProposalExecuted[['block_signed_at','block_height','tx_hash','id']]
    df_final_ProposalExecuted['exicuted'] = True
    df_proposals = pd.merge(df_final_ProposalCreated, df_final_ProposalExecuted, on='id', how="outer", suffixes=('_restaurant_id', '_restaurant_review'))
    praposal_exicuted = dict(df_proposals[df_proposals['exicuted'] == True]['proposer'].value_counts())
    ...
    return (praposers,praposal_created_user,praposal_exicute_user,'Praposal Created','Praposal Exicuted')
...
def get_data():
    ...
    for i in range(1,14544855,1000000):
        url = 'https://api.covalenthq.com/v1/1/events/address/0xc0da01a04c3f3e0be433606045bb7017a7323e38/?page-size=1000000&starting-block=' + str(i) + '&ending-block=' + str(i +1000000) + '&key=ckey_733562fd03494fdab22255ab185'
    ...
    df1 = pd.concat([df.drop(['decoded'], axis=1), df['decoded'].apply(pd.Series)], axis=1)
    df2 = pd.concat([df1.drop(['params'], axis=1), df1['params'].apply(pd.Series)], axis=1)
    return df2
...
def get_voting_data(df):
    ...
    support_true = list(df_final_VoteCast[df_final_VoteCast['support'] == True].groupby(['proposalId']).sum()['votes'])
    support_false = list(df_final_VoteCast[df_final_VoteCast['support'] == False].groupby(['proposalId']).sum()['votes'])
